...
setKV _ _ = throwError "Err invalid key for SET command"
...
getKV v
  | BulkString (Just (_, key)) <- v = do
      ...
      case HashMap.lookup key kv of
        ...
        _ -> return Null
  | otherwise = throwError "Err invalid key for GET commadn"
...
setH tbl key value
  | (BulkString (Just (_, idx))) <- tbl,
    (BulkString (Just (_, k))) <- key = do
      ...
      return $ SimpleString "OK"
  | otherwise = throwError "Err invalid key for HSET command"
  ...
  | (BulkString (Just (_, idx))) <- tbl,
    (BulkString (Just (_, k))) <- key = do
      ...
      case HashMap.lookup k hm of
        Just v -> return v
        _ -> return Null
  | otherwise = throwError "Err invalid key for HGET command"
